<template>
  <van-nav-bar title="货币型基金" left-text="返回" left-arrow @click-left="onClickLeft" />
  <div class="box">
    <van-list
      ...
      style="width: 95%"
    >
      <div class="fundlist" v-for="item in list" :key="item" @click="todetail(item)">
...
<script lang="ts" setup>
  ...
  import { CurrencyDetail, queryFund, SharesDetail } from '@/axios/business/finance.api'
  ...
  function todetail(fund: any) {
    ...
    CurrencyDetail(fund.id).then((value: any) => {
      ...
      store.commit('getRate', value.fundRateList)
