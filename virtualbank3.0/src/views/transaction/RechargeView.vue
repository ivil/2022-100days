...
<script lang="ts" setup>
  ...
  import { rechargeRequest, getBankCardList } from '@/axios/business/transaction.api'
  ...
  const confirmPayPassword = () => {
    if (formData.payPassword.length !== 6) {
      ...
      // 发送请求
      rechargeRequest(submitData).then((value) => {
        loading.close()
        if (value) {
          Toast.success({
            message: '充值成功！',
